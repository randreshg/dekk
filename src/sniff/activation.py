"""Environment activation from .sniff.toml specifications."""
...
from .shell import ActivationScriptBuilder
...
class EnvironmentActivator:
    """Activate project environment from .sniff.toml specification."""

    def __init__(self, spec: EnvironmentSpec, project_root: Path):
        self.spec = spec
        self.project_root = project_root

    def activate(self, shell: Optional[str] = None, use_cache: bool = True) -> ActivationResult:
        ...
        # Add custom env vars and paths from .sniff.toml
        ...
        # Validate tools
        missing_tools = [
            name for name, spec in self.spec.tools.items()
            if not spec.optional and not shutil.which(spec.command)
        ]

        # Generate shell script if requested
        activation_script = None
        if shell:
            activation_script = ActivationScriptBuilder(shell).generate(builder.build().env)
    ...
    def from_cwd(cls) -> EnvironmentActivator:
        """Create activator by finding .sniff.toml from current directory."""
